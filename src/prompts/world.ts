...
注意：本卷有 ${config.generation.chaptersPerVolume} 章，输出较长。请严格控制每章 summary 在 80-120 字以内，keyScenes 最多 2 条。确保 JSON 完整闭合，不要被截断。
...
2. 每章 summary 必须控制在 80-120 字，不要写太长
3. keyScenes 每章最多 2 条
4. 章节之间有连贯性和递进感，结尾留钩子
5. 节奏控制：铺垫→冲突→高潮→回落→新悬念
6. 确保角色行为符合其性格设定
7. 本卷的剧情要服务于当前篇的子冲突
8. **最重要的是 JSON 必须完整闭合，不要被截断**
